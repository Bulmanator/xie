...
static LPWSTR win32_utf8_to_utf16(xiArena *arena, string str) {
    LPWSTR result = 0;

    // @todo: this should return a string instead of a direct pointer

    DWORD count = MultiByteToWideChar(CP_UTF8, 0, (LPCCH) str.data, (int) str.count, 0, 0);
    if (count) {
        result = xi_arena_push_array(arena, WCHAR, count + 1);
...
static void win32_game_code_init(xiWin32Context *context) {
    ...
    string dll_name = xi_str_wrap_const(L"\\game.dll");

    buffer src_path;
    src_path.used  = 0;
    src_path.limit = exe_path.count + dll_name.count + 2;
    ...
    xi_buffer_append_bytes(&src_path, 0, 2);
...
static b32 win32_game_code_is_valid(xiWin32Context *context) {
    b32 result = (context->game.init != 0);
    return result;
}
...
static void win32_xi_context_update(xiWin32Context *context) {
    ...
    if (!context->window.user_resizing) {
        ...
        if (xi->window.state != XI_WINDOW_STATE_FULLSCREEN) {
            ...
            if (window_width != xi->window.width || window_height != xi->window.height) {
                LONG style       = GetWindowLongW(context->window.handle, GWL_STYLE);
                ...
                if (GetDpiForMonitor(monitor, MDT_EFFECTIVE_DPI, &xdpi, &ydpi) == S_OK) {
                    ...
                    if (AdjustWindowRectExForDpi(&client_rect, style, FALSE, 0, xdpi)) {
                        ...
                        SetWindowPos(context->window.handle, HWND_TOP, 0, 0, window_width, window_height, SWP_NOMOVE | SWP_NOZORDER);
                    }
                }
            }
        }
        ...
        if (xi->window.state != context->window.last_state) {
            LONG old_style = GetWindowLongW(context->window.handle, GWL_STYLE);
            ...
            if (context->window.last_state == XI_WINDOW_STATE_FULLSCREEN) {
                ...
                // entering fullscreen
                //
                SetWindowPlacement(context->window.handle, &context->window.placement);
            }
            ...
            if (xi->window.state == XI_WINDOW_STATE_FULLSCREEN) {
                ...
                //
                if (GetWindowPlacement(context->window.handle, &context->window.placement)) {
                    HMONITOR monitor = MonitorFromWindow(context->window.handle, MONITOR_DEFAULTTONEAREST);
            ...
            SetWindowLongW(context->window.handle, GWL_STYLE, new_style);
            SetWindowPos(context->window.handle, HWND_TOP, x, y, w, h, flags);
    ...
    if (!xi_str_equal(xi->window.title, context->window.last_title)) {
        ...
        SetWindowTextW(context->window.handle, title);
...
static DWORD WINAPI win32_main_thread(LPVOID param) {
    ...
    if (RegisterClassW(&wnd_class)) {
        ...
        if (xi_str_is_valid(xi->window.title)) {
            title = win32_utf8_to_utf16(temp, xi->window.title);
        }
        else {
            title = L"xie";
        }
        ...
        if (hwnd != 0) {
            ...
            // we have to reset the state in the case of borderless windows because windows
